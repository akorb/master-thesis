...
This has to avoided by integrating the good-practices with working with a \ac{TPM}, which includes having secrets stored also elsewhere. % TODO: Maybe cite something, I feel like Microsoft has something for that
...
However, it should be noted that although this protects the fTPM from downgrade attacks, it does not protect its data from downgrade attacks.
However, this is usually achieved by storing the data on a Replay Protected Memory Block~(RPMB) partition.
This is the reason why a RPMB partition is part of Microsoft's hardware requirements for a firmware TPM~\cite{Raj2015}.
