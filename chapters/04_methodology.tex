...
In between can be an arbitrary number of Alias certificates for ``ordinary'' firmware components.
...
So, this EK certificate needs to fulfill the requirements for an Alias certificate from the DICE specification \cite{TCGAttestation2021} and the EK certificate requirements from the TPM specification \cite{tcg-ek}.

Therefore, we need to ensure that these two specifications declare no conflicting requirements.
We only consider restrictions for the X.509 fields, which are hard requirements, and common fields which can appear in both specifications.
We assume the firmware to not having access to a secure clock, keeping the requirements low.
Also, we only name the requirements which specifically apply to our certificate.
For example, it is a leaf of the certificate chain, i.e., no CA.

We present the result of our comparison in \autoref{tab:cert_comparison}.
In summary, there are mostly no conflicts since both certificates expect the same value, both requirements can be satisfied with one value, or only one of the certificates dictates a restriction for a specific field.
...
A general term like ``fTPM'' is prohibited by the EK certificate specification.
...
This ensures that the EK certificate is also as expected for systems that do not know our solution and do not know the TCI part of the certificates.
...
However, we omit this extension because if a verifier trusts a TCI, he should also know which manufacturer, exact code and TPM specification it conforms to.
Furthermore, the TCI is more accurate and reliable because it is an exact measurement of the firmware TPM rather than relying on information propagated by the TPM.
...
This cannot happen with the TCI that is part of the certificate chain, as this malicious firmware component would be detected.
