...
Establishing the storage key also prevents an attacker from accessing an fTPM's data by downgrading it\@.
For example, if an attacker wants to access data stored in a particular fTPM and can exploit a vulnerability of an earlier version of that fTPM, it is not possible to replace the fTPM with its old version, as neither the attacker itself nor the fTPM itself will be able to decrypt the previous data.

However, it should be noted that this does not protect the fTPM data from a rollback attack.
This is normally achieved by storing the data on a Replay Protected Memory Block~(RPMB) partition.
