...
This is to be expected, since our system uses \ac{DICE}\@.
We leverage the \ac{DICE} as our \ac{SRTM}.
...
Like any other \ac{DICE} component, the fTPM receives its secret \ac{CDI} from its predecessor layer.
Recall that the \ac{CDI} is tied to the identity of the fTPM including the entire underlying firmware stack and the \ac{UDS}\@.
Two values are derived from the \ac{CDI}\@.
...
And since each Alias public key is device unique, as it roots in the device's unique \ac{UDS}, the verifier would need to know all Alias public keys for each component on every device he trusts, which we consider unrealistic.
...
Thereby, the encryption key is temporarily stored in plaintext on the hard drive, which is consequently restored on the TPM after its update.
...
It is guaranteed by the fact that the prover does not transmit any data derived from its \ac{UDS} to the verifier.
