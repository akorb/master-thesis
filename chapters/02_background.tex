...
It typically consists of two parts \cite{McCune2008}. (i) The attestation, and (ii) establishing a secure channel. In this work, we focus on the first step.
...
Note that we are aware that 'PCR register' is a RAS\footnote{Redundant Acronym Syndrome}, but we have chosen to leave it as such for clarity, as 'PC register' can be associated with other meanings.
...
As of now, a fTPM is strictly bound to the processor manufacturer, such that you can trust the underlying firmware as well which is provisioned by the manufacturer, e.g., Intel, too. For example, common implementations are the Intel® Platform Trust Technology~(Intel PTT) \cite{intelProcessorSecurity}, and AMD's Secure Processor~(AMD-SP), which in fact is an ARM-based coprocessor on the die which enables ARM's TrustZone \cite{Khalid2020}.

Running on the main processor, e.g., a fully-fledged Arm Cortex core, entails an advantage and a disadvantage. On the one hand, running on the same processor as the rest of the system means less isolation, while a \ac{dTPM} brings its own processor that is completely isolated from the main processor. On the other hand, however, this processor is generally weak \cite{Goh2013, Raj2015}. Raj et al. \cite{Raj2015} and Cheng et al. \cite{Cheng2020} independently concluded that the firmware-based modules are generally much faster after comparing the performance of \acp{fTPM} and \acp{dTPM}.

However, there are also disadvantages. First, they cannot provide true RNG, since hardware is required for that.
...
Second, they are started later in the hosts' boot chain than a dTPM that is accessible from the beginning. This has the consequence that the hashes of the components booted before the fTPM cannot be sent to the fTPM. Last, fTPMs depend on more components for its security than single-component dTPMs, e.g., the \ac{TEE}, and the boot chain.
...
%Uses sealing functionality of TPMs, therefore, bound to the exact platform.

Secure Boot and Measured Boot are often used in conjunction.

\section{Device Identifier Composition Engine}

To the best of our knowledge, \ac{DICE} is so far considered a secure concept apart from physical attacks, only implementation problems can mean security problems.

% Originally defined by Microsoft
% Now standardized by TCG
