...
% Winter and Dietrich \cite{Winter2013} continue the 'TPM Reset Attack' by circumventing counter-measurements taken since the reveal of the attack. Therefore, it also focuses on resetting the PCR values by active hardware attacks, while the running platform does not change. For that, a valid boot procedure is required to retrieve the valid PCR values. However, it also introduces the opposite, where the PC is reset, and a malicious operating system/firmware booted. The attacker prevents the TPM from receiving the reset signal, keeping the valid PCR values of the previous boot procedure. While its conceptually easier since the attacker does not need to know the valid PCR values since they are already in-place, it requires active manipulation of the data transmitted via the LPC bus to shield the TPM from the reset signal.
