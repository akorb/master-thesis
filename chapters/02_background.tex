...
Each measurable layer is represented by a certificate forming a certificate chain.
For layers $n \geq 1$ they are called Alias certificates, while for Layer 0 it is called DeviceID certificate.
The differing names come from their different purposes. An AliasCert merely represents the identity of a layer, i.e., represents an alias of the component.
A DeviceID certificate is additionally the asset which allows a long term identification of the device.
...
Another important difference between the AliasCert and DeviceID certificate is, that the DeviceID certificate is generated during provisioning by a private key of the manufacturer.
This allows linking the DICE implementation to a manufacturer, which is important to retrieve the guarantees the manufacturer conducts for its DICE implementation.
A DeviceID certificate contains the TCI of Layer 0.

% Add certificate graph chain, rooting from manufacturer, show which TCI is contained. Maybe show with colors what is generated during boot time, and what is externally generated (Manufacturer cert & DeviceID cert)

Trust on CDI based on secrecy of UDS.
How to establish trust into the UDS? DeviceCert

% From 8.1.3 Layered Architecture
% The DICE TCB layer that generates an IDevID (typically layer 0 = DeviceID cert) SHOULD be either immutable or mutable under the control of the device manufacturer
% This is because it is required that any change to the layer 0 TCB MUST result in a different DeviceID key. Which changes identity of device.
