...
%A DeviceID certificate is additionally the asset which allows a long term identification of the device, and provides device provenance, i.e., links the DICE to its manufacturer.Another important difference between the AliasCert and DeviceID certificate is, that the DeviceID certificate is generated during device provisioning by the manufacturer.
...
% What happens if Layer 0 changes? DeviceID cert is pre-provisioned, how is it detected?
...
% While the alias key pair is always generated by the preceding layer, the DeviceID key pair is generated within layer 0 itself, and the deviceID certificate contains its own TCI.
%The DeviceID certificate is not generated during ordinary boot-times, but in a manufacturer controlled provisioning-process.

% So, the DeviceID certificate contains the TCI of layer 0, however, it is pre-provisioned by the manufacturer.


\input{elements/figure_dice_certs.tex}
