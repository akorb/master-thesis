...
A typical environment for \acp{fTPM} are \acp{TEE} isolating them from the \ac{REE}, which means that even the OS in the REE cannot arbitrarily access the memory of the fTPM, but must adhere to the \ac{fTPM}'s and \ac{TEE}'s provided interfaces.
...
Nonetheless, plain Secure Boot does not prevent downgrade attacks, since only the authenticity, but not the concrete versions of boot components are verified~\cite{272306}.
