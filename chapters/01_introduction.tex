...
In a typical setup, this component is immutable without the involvement of its manufacturer that reveals itself to a verifier by signing and storing a certificate on the helper components supplied by it.
This allows the verifier to establish trust to the helper component by knowing its manufacturer.
...
Instead, it is sufficient to trust the independent manufacturer of the hardware root of trust, which in contrast requires minimal assumptions, such as the absence of side-channel vulnerabilities.
