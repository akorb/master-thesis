...
The main threat is trusting a remote firmware TPM which is in fact not trustworthy.
...
% The main threat is the modification of the binary of the fTPM before or during boot.
% For example, by exchanging the SD card storing the binary.
...
\begin{itemize}
  % \item{\textbf{Establish trustworthiness to firmware TPM}}
  ...
  % DICE, its design, not concrete HW, its protection of the UDS
  ...
  % TA UUID is public.
  % But they cannot know its CDI, since the OP-TEE OS does not allow measuring of arbitrary applications, but TA's can only measure themselves.
  % This prevents other TA's from generating the fTPM's CDI, and subsequently its storage key.
  % The trusted OS, e.g., OP-TEE OS can potentially access the fTPM's memory and thereby extract the storage key, but the trusted OS is also measured as part of our certificate chain and therefore be declared as trustworthy, which implies it does not abuse its privileges.
  ...
  % \item{\textbf{Identification of the fTPM stack};\\
  % The verifier should be informed of the exact versions of each component of the fTPM stack in a trustworthy manner, so that the verifier can independently decide whether to trust the fTPM.}\label{enum:tpm-id}
  % %ID is part of certificate chain. And TPM can prove that the certificate chain represents it by proving being in control of EKpriv.

  % \item{\textbf{Confidentiality and integrity of TPM storage};\\
  % Data should not be accessible or modifiable outside of TPM access controls, i.e., the TPM commands defined by its specification~\cite{tpm}.}
  % % Also not to other TA's, which can spoof UUID. But not possible, since CDI of fTPM unknown and can then not decrypt data.

  % \item{\textbf{Data of TPM bound to compound identity of fTPM stack};\\
  % The current storage of the fTPM should be only accessible to the identity of its entire stack which includes its underlying firmware components.
  % Conversely, as soon as a component that could affect the fTPM is modified, the fTPM with then a new identity should not have access to the previous data anymore.
  % This includes an update of any relevant component.}
