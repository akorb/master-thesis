...
They require a TEE, storage hardware supporting Replay Protected Memory Block~(RPMB), a secure world hardware fuse, and a secure entropy source~\cite{Raj2015}.
RPMB is for example provided by eMMC and UFS\@.
The eMMC is a storage type, where storage has to present anyway.
So, it does not involve extra hardware, but a constraint for the storage type. % hat man echt eMMC allein?
